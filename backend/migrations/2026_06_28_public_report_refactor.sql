-- =====================================================================
-- Public Secure Student Report Refactor RPC Function
-- Proposes update to get_public_report to query from the refactored
-- attendance_records/attendance_sessions and student_ledger tables.
-- =====================================================================
...
DECLARE
  ...
  v_result JSONB;
BEGIN
  -- Fetch student details using the new relational group schema
  ...
  -- Fetch Attendance Statistics (from new refactored attendance_records table)
  ...
  -- Fetch Detailed Grades History (with class_average calculation)
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', id,
      'type', type,
  ...
  -- Fetch Detailed Attendance History (from new refactored tables)
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', a.id,
      'date', a.date,
  ...
  -- Fetch Detailed Payments History (from student_ledger table)
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', id,
      'amount', amount,

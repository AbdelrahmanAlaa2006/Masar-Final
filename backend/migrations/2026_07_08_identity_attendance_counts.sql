-- Fix get_student_identity attendance reporting:
--   * Return real counts (present / late / absent / total sessions) so the scan
--     modal can show "حضر X من Y حصة" instead of a bare percentage.
--   * A student with no attendance records now gets NULL percentage instead of
--     a fake 100%, and is no longer eligible for the excessive_absences warning.
